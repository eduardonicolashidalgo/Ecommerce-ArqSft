...
public class ProductREST {
	...
	public ResponseEntity<WrapperResponse<ProductDTO>> getAllProducts() {
		try {
			...
			WrapperResponse<ProductDTO> response = new WrapperResponse(true, "Consulta exitosa", products);

			return new ResponseEntity<>(response, HttpStatus.OK);
		} catch(Exception e) {
			e.printStackTrace();
			logger.error(e.getMessage());
			WrapperResponse<ProductDTO> response = new WrapperResponse(false, "Error al consultar los productos");
			return new ResponseEntity<>(response, HttpStatus.OK);
		}
	}
